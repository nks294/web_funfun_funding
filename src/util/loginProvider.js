...
export const LoginProvider = ({ children }) => {
    ...
    const [usrAuthLevel, setUserAuthLevel] = useState(() => {
        const savedValue = sessionStorage.getItem('context');
        return savedValue && JSON.parse(savedValue).usrAuthLevel !== undefined
            ? JSON.parse(savedValue).usrAuthLevel
            : 0;
    });
    ...
    }, [isLoggedIn, usrNickName, usrEmail, isMainPage, usrAuthLevel]);
    ...
    useEffect(() => {
        ...
        if (savedValue) {
            ...
            if (parsedValue.usrAuthLevel !== undefined) {
                setUserAuthLevel(parsedValue.usrAuthLevel);
            }
        }
    }, []);
